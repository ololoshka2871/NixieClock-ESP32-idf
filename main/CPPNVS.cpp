 ...
 * @param [in] openMode The open mode.  One of NVS_READWRITE (default) or
 * NVS_READONLY.
 */
NVS::NVS(std::string name, nvs_open_mode openMode) {
  esp_err_t errRc = ::nvs_flash_init(); // Initialize flash
  if (errRc != ESP_OK) {
    ESP_LOGE(LOG_TAG, "nvs_flash_init: rc=%d" /*" %s"*/, errRc
 ...
 * @brief Desctructor
 */
NVS::~NVS() { ::nvs_close(m_handle); } // ~NVS

/**
 * @brief Commit any work performed in the namespace.
 */
void NVS::commit() { ::nvs_commit(m_handle); } // commit

/**
 * @brief Erase ALL the keys in the namespace.
 */
void NVS::erase() { ::nvs_erase_all(m_handle); } // erase
...
int NVS::get(std::string key, uint8_t *result, size_t &length) {
  ESP_LOGD(LOG_TAG, ">> get: key: %s, blob: inputSize: %d", key.c_str(),
           length);
...
void NVS::set(std::string key, std::string data, bool isBlob) {
  ESP_LOGD(LOG_TAG, ">> set: key: %s, string: value=%s", key.c_str(),
           data.c_str());
  if (isBlob) {
